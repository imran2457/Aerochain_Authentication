...
def InitializeNetwork(nod, datarate, packetSize):
    ...
    return nod, packetSize, 10.0, 5.0, 1.0, 0.5, 10000, 100
...
def main():
    ...
    nod = 3; datarate = "54"; packetsize = 512

    # NEW: CRL manager rooted at the blockchain/log directory
    rm = RevocationManager(directory_log)

    print("\nManipulating Network and ready to collect network data.")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        ans = input().lower()
        rm = RevocationManager(blockchainDatadirectory)

        if ans in ('yes','y'):
            # Authentication (with CRL gate)
            secret_key = generate_secret_key()
            save_secret_key_to_file(secret_key, secretKeydirectory)
            new_uav_id = f"UAV-{nod+1:02d}"
            IsAuthenticated = authenticate_node(secretKeydirectory,
                                               incoming_uav_id=new_uav_id,
                                               revocation_base_dir=blockchainDatadirectory)
            if IsAuthenticated:
                print(f"[OK] {new_uav_id} authenticated.")
                nod += 1
            else:
                print(f"[FAIL] {new_uav_id} authentication failed.")

        else:

            target = "UAV-01" if nod >= 1 else "UAV-00"
            blk = rm.revoke(target, "Demo: operator request")
            print(f"Revocation issued for {target} at block {blk}. Evidence under: {blockchainDatadirectory}")
