...
class Block:
    def __init__(self, block_data, index):
        self.block_data = block_data
        self.index = index
        self.timestamp = datetime.now().isoformat()
        self.block_hash = hex(abs(hash((self.block_data, self.index, self.timestamp))))
...
class Blockchain:
    ...
    def generate_genesis_block(self):
        self.create_block_from_transaction("GENESIS")

    def create_block_from_transaction(self, tx_data):
        idx = len(self.chain)
        b = Block(tx_data, idx)
        self.chain.append(b)
        return b
    ...
    def get_chain(self):
        return [f"{b.block_hash}___{b.block_data}___{b.index}___{b.timestamp}" for b in self.chain]

    # NEW: append a revocation record "on-chain"
    def append_revocation_tx(self, uav_id: str, reason: str):
        tx = {"type":"REVOKE","uav_id":uav_id,"reason":reason,"ts":datetime.now().isoformat()}
        return self.create_block_from_transaction(str(tx))